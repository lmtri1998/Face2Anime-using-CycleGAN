...
### What is GAN ? ###
Generative Adversarial Networks (GANs) are an approach to generative modeling using deep learning methods. Generative modeling are subset of unsupervised learning, a type of Machine Learning algorithm, that involves self learning patterns in input data in order to generate new examples that could have been from the traing dataset. Out of all training methods for a generative model, GAN seemed to me to be the most creative and clever way. It turned the unsupervised learning problem into a supervised learning problem by introducing two sub-models, a generator and a discriminator. The generator network and discriminator network are jointly trained against each other in a zero-sum game. While the generator tries to produce samples from the domain of the desired distribution, the discriminator is actively trying to classify whether a sample is from the actual distribution (real) or is generated by the generator (fake). Eventually, the generator will learns to aproximate the underlying distribution and left the discriminator guessing randomly. I am going to quickly go over the GAN objecttive below since CycleGAN is built on top of it.

<p align="center">
  <img src="./resources/fig4.-Objective-function-in-GAN-formulation.png" width="100%">
 <figcaption>Figure 0 - GAN objective function</figcaption>
<!--   <img src="your_relative_path_here_number_2_large_name" width="350" alt="accessibility text"> -->
</p>

From Figure 0 above, the generator and discriminator is call G and D respectively. Pz is the probability distribution of the latent space which is usually just a Gaussian distribution. The distribution of our training dataset is Pdata here, in our project it will be the probability distribution of the 128x128 images. When we sampled x from Pdata, the discriminator will wants to predict that the sample is a real sample. When the generated sample, G(z), is given to our discriminator, it will want to be classified as fake. The objective here is simple, the discriminator D will wants to maximize log(1-D(G(z))) to turn the likelihood of D(G(z)) to 0 [7]. On the other hand, the generator G wants to drive D(G(z)) to 1 so that the discriminator will mistakenly classified its generated output as real. So, generator G would want to minimize log(1-D(G(z))[7]. Note that when D(G(z)) output 0 it means that it classified the input as fake, while when D(G(z)) output 1 it means that it classified the input as real. So this is how the generator network and discriminator network are jointly trained against each other in a zero-sum game.
